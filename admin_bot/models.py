...
class MessageCopy(Base):
    ...
    recipient_participant_id = Column(BigInteger, ForeignKey("participant.id", ondelete="CASCADE"))
    recipient_tg_message_id = Column(BigInteger)  # те, що отримав конкретний учасник
    senders_tg_message_id = Column(BigInteger, nullable=True)  # те, що було у відправника
    created_at = Column(DateTime, server_default=func.now())
